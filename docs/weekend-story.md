...
テンプレートは既存の公式アセット `public/icon-512.png` をヘッダーとフッターへ配置し、ブランドの紙色、白地、墨色、朱色だけを基調とする。イベントカードは、左側に団体名・タイトル・参加条件、右側に会場・地域・参加費・アクセスを置く。明朝体の見出し、朱色の日時ラベル、装飾罫線、細いカード枠で既存OGPと統一する。イベント由来の文字列は正規化・言い換えせず、参加費とアクセスは値がある場合だけ表示する。未確認の団体ロゴ、剣道シルエット、和柄、独自アイコンは使用しない。

## フォント

本文用の `assets/fonts/NotoSansJP[wght].ttf` と見出し用の `assets/fonts/NotoSerifJP[wght].ttf` を明示的に読み込む。OSのフォント探索や暗黙のフォールバックは行わない。
...
Noto Serif JPもGoogle Fonts公式リポジトリから無改変で同梱し、`assets/fonts/OFL-NotoSerifJP.txt` にSIL Open Font License 1.1を保持する。
