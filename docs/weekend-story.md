...
テンプレートは既存の公式アセット `public/icon-512.png` をヘッダーへ配置し、ブランドの紙色、白地、墨色、朱色だけを基調とする。日付を朱色のラベル、参加条件を淡い朱色のピル、イベントカードを朱色のアクセント線と控えめな影で表示する。ページ内のイベント数に応じてカード間の余白を調整するが、イベント由来の文字列は正規化・言い換えしない。アクセスは値がある場合だけ表示する。
